...
// ── 4개 채널 영점 대비 변화폭의 평균값(대표값) 반환 ──
// (단일 센서 실험 데이터로 도출한 기존 임계값과의 스케일 연속성을 위해 average 사용.
//  4개 중 하나만 튀는 노이즈가 대표값에 그대로 반영되지 않고 상쇄됨)
...
void loop() {
  ...
  float tiltPct    = 0;
  for (uint8_t ch = 0; ch < NUM_GYRO; ch++) {
    tiltPct += tiltPctArr[ch];
  }
  tiltPct /= NUM_GYRO;   // 4채널 평균을 대표 tiltPct로 사용
